...
platform permissions require `acr=mfa` and `auth_time` no older than 300
seconds. Google and Entra are upstream authentication providers whose
identities must resolve through the stable Tequity subject registry. Resource
...
remain behind the exact environment-specific Vault references declared in
`src/identity.ts`. Local Docker Compose does not depend on Pulumi or ESC.

Cloud preview and apply remain separate from credential-free validation.
Production apply requires a separate explicit human security approval.
